...
import { basicQuestions, headacheQuestions, drugQuestions } from "./data/questions.js"
...
function setupDairyDate() {
  const currentDate = new Date();
  let mockDate = new Date().setDate(currentDate.getDate() - 10);
  localStorage.setItem("headacheDiaryStartDate", mockDate);
  console.log('установлена текущая дата')
}
...
function submitForm() {
  ...
  if (localStorage.getItem("headacheDiaryStartDate") === null) {
    setupDairyDate();
  }
}
